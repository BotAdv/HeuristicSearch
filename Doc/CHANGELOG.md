...
### 修复：`hidden` 被组件 `display` 覆盖 → 参数栏其实没被隐藏

* `.toolbar { display: flex }` 会盖掉 UA 样式表的 `[hidden] { display: none }`
  （作者样式表优先于 UA 样式表），`el.hidden = true` 只改属性、视觉无变化。
  已在 `app.css` 顶部加全局兜底 `[hidden] { display: none !important; }`。
* 实测：收起时 `display:none` / 高 0px，按 F9 变 `flex` / 77px。
* 守卫用例同时断言“兜底规则存在”与“`.toolbar` 仍是 `display:flex`”。

### 布局

* **沙盒单策略页签两列铺满**：`.sandbox` 由 3 列改为
  `minmax(0,1.05fr) minmax(0,1fr)`，右侧不再预留空位（≤1080px 退化为单列）。
* **多策略对比公共操作条**：`开新局 / 所有策略各走一步 / 自动跑完` 从工具栏搬到三个面板正上方的
  `.bar-common`，与对局状态行同一行（左状态、右按钮）。
* **逐步猜测页每行 2 列**：`.human` 改 `repeat(2, minmax(0,1fr))`（≤1080px 单列）；
  对局历史因此落在**本轮猜测正下方**（第 2 行第 1 列）；删掉已无定义的 `panel wide` 类。

### 策略库：卡片内联轨迹

* 每张卡片拆成 `.card-main`（原内容）+ `.card-trace`（轨迹，默认收起）；
  试玩后卡片加 `.expanded` → `grid-column: span 2`，内部变「参数 | 轨迹」两列，
  于是**轨迹出现在卡片右侧**，同排其它卡片自动顺延；≤1280px 退回单列。
* 顶部**取消**「试玩策略」下拉框；**跑一局** = 所有卡片用各自参数各跑一局并全部展开轨迹
  （状态行汇总每张卡片轮数）；新增**隐藏轨迹**按钮 = 全部收起。
* 每张卡片仍保留「快速试玩」（只跑这一张），轨迹表头带「收起轨迹」可单张折叠。
* 删除页面底部公共的 `#tracePanel`。
* `/api/quickplay` 返回体只有 `strategy`（key），汇总时中文名要取前端策略表，否则出现 `undefined`。

### 测试

* `tests/test_ui_matrix.py` 新增 `TestLayout18` 6 个用例（hidden 兜底、两列布局、
  公共操作条、策略库顶部只剩两个按钮与内联轨迹契约）。总数 112 → **118 例全部通过**。
* 新文档 [`Doc/17_布局与轨迹呈现调整.md`](17_布局与轨迹呈现调整.md)。

## [1.7.0] — 2026-09-21

### 新增：「历史序列」合并卡片 + 上三角矩阵 + 沙盒状态保持

* **猜测序列与对局历史合并为「历史序列」卡片**：上半 = 对局历史（每轮一行 + 「理由」按钮），
  下半 = 猜测序列（`1 行 10 列`，与历史行同构），槽位颜色即该位反馈、`🔒` 即已锁定位置。
* **动作按钮搬到卡片右上角**：手动试玩 = **开新局 / 提交猜测 / 让策略预测 / 清空槽位**，
  策略自动 = **开新局 / 走一步 / 自动跑完**；顶部参数栏只剩参数本身。
* **F9 切换参数栏**：手动试玩页签默认收起（只留一行提示），策略自动页签默认展开；
  `Enter` 等价于「走一步 / 提交猜测」。（`static/js/sandbox.js`、`static/css/app.css`）
* **矩阵只保留上三角**：`a > b` 的 45 格与「被删除的 10 个组合」一样 `disabled`，
  且**共用同一条 CSS 规则**（外观完全一致）；100 格中 45 格可点、55 格置灰。
  逐步猜测页同样生效。（`static/js/combogrid.js`）
* **点选交互**：左键选中（默认依次填入，先点槽位可指定填入位置）、右键取消（矩阵格 → 从猜测序列移除，
  槽位 → 清空该位）；10 位已满且未指定位置时不再静默覆盖，改为提示。
* **选中高亮改为闪烁**：本轮选中的格子不再画蓝色框线，而是按自身状态色高亮 + `@keyframes cg-blink` 呼吸。
* **沙盒状态保持**：`localStorage['hs.sandbox.v1']` 记住当前页签、参数栏展开状态、三个页签的
  对局 id / 槽位 / 待填位置 / 参数与多策略计划清单；刷新后自动恢复，
  会话失效时提示并降级（`clearGameInStore`）。手动改动后上一次「让策略预测」的解释标注**【已过期】**。

### 修复

* `firstFreeSlot()` 满行返回 `-1` 时，提示语会显示“下一个填入位置已指定为**第 0 位**”
  → 新增 `nextTarget()` 把 `-1` 归一到 `null`。
* `.cg-off` 曾单独 `opacity: .55`，与「被删除组合」的计算样式不一致 → 合并为同一条规则。

### 测试

* `tests/test_ui_matrix.py` 新增 4 个测试类 12 个用例：合并卡片结构 / 手动 4 按钮（含「开新局」）/
  策略 3 按钮 / 导航栏不再有动作按钮 / F9 默认收起；上三角开关 / 右键取消 / 闪烁而非蓝框 /
  依次与指定填入；`hs.sandbox.v1` 生命周期 / 会话失效降级 / 三页签都接入。总数 100 → **112 例全部通过**。
* 新增 `tools/run_tests_quiet.py`：只输出摘要，失败时给「用例名 + 断言首行」。
* 新文档 [`Doc/16_历史序列卡片与状态保持.md`](16_历史序列卡片与状态保持.md)。

## [1.6.0] — 2026-09-21

### 新增：沙盒三个子页签 + CORRECT 位置锁定

* 沙盒页改为**三个子页签**（① 手动试玩 / ② 策略自动 / ③ 多策略对比），
  每个页签带**自己的工具条与状态栏**，所以「所有策略各走一步」这类按钮
  永远和它控制的卡片同屏，不用再回到页面顶部找。（`templates/index.html`、`static/css/app.css`）
* 每个页签的卡片是**独立 DOM 与独立状态**：`sandbox.js` 拆为 `SingleView`（手动/自动各一个实例）
  与 `MultiView`（每策略一张卡片），互不影响；切页签不丢状态。
* **CORRECT 位置锁定**（所有模式）：某位一旦判定 CORRECT，
  该位置在沙盒里不可改选其它组合（槽位点不动、矩阵点选跳过、清空只清未锁定项），
  矩阵与槽位都打 🔒；多策略卡片各自显示自己的锁定位置；
  逐步猜测页对锁定位置显示 🔒 并自动预填 C（仍可修正）。锁定完全由历史推导，撤销后自动解除。

### 移除：两个不使用反馈的策略

* 删除 `random`（均匀随机）与 `fixed_cycle`（固定循环平移），理由：完全不使用反馈、
  在 $45^{10}$ 空间里不可能解出（只占满轮次上限），且会在**同一位置反复填同一个错误组合**。
* 同步更新：策略注册表与 `DEFAULT_PLAN_KEYS`、命令行基准 `greedy_vs_adaptive` 预设、
  `/api/experiments/presets`、策略库页（现 5 张卡片）、`Doc/01/03/06/09/14`、`README`。
* 历史日志（`Doc/08`、`Doc/06` 旧结果表）保持原样，不重写当时跑出的数据。

### 测试

* `tests/test_ui_matrix.py` 新增：三个页签与三个独立卡片容器的装配契约、锁定类名/函数已接线、
  “两个不使用反馈的策略确已移除”。单元测试 98 → **100 例**。
* 新文档 `Doc/15_子页签与位置锁定.md`。
